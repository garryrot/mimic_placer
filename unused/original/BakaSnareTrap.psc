...
state Open
	...
	actorref = TriggerRef as actor
		...
		if checkPerks(TriggerRef) && actorref
			...
			GoToState("Busy")
			Bool bTrapActiavted = (TNTRController as TNTRControllerScript).SetTrapindex(actorref, self)
			if bTrapActiavted
	...
	Event OnBeginState()
		...
		if QTE && (actorref == playerref);This is where QTE is placed
			...
			if randomi == 1
				...
				playAnimation("TriggerC01")
				WaitForAnimationEvent("PlayerMoan")
				PlayVoice(ActorRef, 80, 1, 3.0)
				WaitForAnimationEvent("TransC01")
				goToState("Escaped")
			else
